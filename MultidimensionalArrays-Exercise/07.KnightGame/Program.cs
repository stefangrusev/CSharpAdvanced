...
namespace _07.KnightGame
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < board.GetLength(0); i++)
                {
                    ...
                    {
                        CheckForTheKnightIsItInside(board, ref maxAttacked, ref row, ref col, i, j);
                    }
                }
        ...
        private static void CheckForTheKnightIsItInside(char[][] board, ref int maxAttacked, ref int row, ref int col, int i, int j)
        {
            if (board[i][j] == 'K')
            {
                int counter = 0;
